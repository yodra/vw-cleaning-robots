...
class RobotTest {
    ...
    void shouldTurnRight() {
        Robot robot = Robot.initialization(Position.of(5, 5), Direction.N);
    ...
    void shouldMoveFollowingTheInstructions() {
        Robot robot = Robot.initialization(Position.of(1, 2), Direction.N);

        robot.execute("LMLMLMLMM", Workspace.create(5, 5));

        assertEquals(Position.of(1, 3), robot.getPosition());
